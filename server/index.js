...
app.post('/api/users/:id/report-answers', express.json(), async (req, res) => {
  ...
  try {
    ...
    // Just persist the answers here and respond immediately - the AI report
    // itself is generated by a separate, follow-up call to
    // /api/users/:id/generate-report (see PatientDashboardPage's post-submit
    // flow), so the survey modal isn't stuck showing a spinner for however
    // long the Python backend call takes. Vercel keeps this function alive
    // exactly as long as the response is pending, so splitting into two
    // sequential awaited requests from the client is safe here in a way a
    // true fire-and-forget background task after res.json() would not be.
...
// ─────────────────────────────────────────────────────────────────────────────
// Generate AI Report Route (follow-up to Submit Report Answers, above)
// ─────────────────────────────────────────────────────────────────────────────
app.post('/api/users/:id/generate-report', express.json(), async (req, res) => {
  const userId = req.params.id;
